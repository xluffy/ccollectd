...
class MongoDB(object):
    ...
    def do_server_status(self):
        ...
        if 'wiredTiger' in server_status:
            for k, v in server_status['wiredTiger']['cache'].items():
                if "bytes currently in the cache" in k:
                    self.submit('gauge', 'wiredTiger.cache.' + 'byte_current_in_cache', v )
                elif "maximum bytes configured" in k:
                    self.submit('gauge', 'wiredTiger.cache.' + 'maximum_bytes_configured', v )
                elif "tracked dirty bytes in the cache" in k:
                    self.submit('gauge', 'wiredTiger.cache.' + 'tracked_dirty_bytes_in_cache', v )
                elif "unmodified pages evicted" in k:
                    self.submit('gauge', 'wiredTiger.cache.' + 'unmodified_pages_evicted', v )
                elif "modified pages evicted" in k:
                    self.submit('gauge', 'wiredTiger.cache.' + 'modified_pages_evicted', v )
                else:
                    continue
